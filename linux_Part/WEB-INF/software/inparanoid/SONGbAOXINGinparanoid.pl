...
$softwarepath="/var/lib/tomcat7/ppipp/ROOT/WEB-INF/software/inparanoid/";

#$softwarepath="/home/baoxing/interprologfORJAVA/inparanoid/";
$blastall = $softwarepath . "blastall";  #Add -aN to use N processors
$formatdb = $softwarepath . "formatdb"; 
$seqstat = $softwarepath . "seqstat.jar";
$blastParser = $softwarepath . "blast_parser.pl";
...
$conf_cutoff = 0.5;   # Include in-paralogs with this confidence or better   #
...
$fasta_seq_fileB = "TARGET";
$outputfile = $filePath . "Output." . $fasta_seq_fileA . "-" . $fasta_seq_fileB;
if ($output){
    open OUTPUT, ">$outputfile" or warn "Could not write to OUTPUT file $filename\n";
}                                           
my $blast_outputAB = $filePath . $fasta_seq_fileA . "-" . $fasta_seq_fileB;
my $blast_outputBA = $filePath . $fasta_seq_fileB . "-" . $fasta_seq_fileA;
my $blast_outputAA = $filePath . $fasta_seq_fileA . "-" . $fasta_seq_fileA;
my $blast_outputBB = $softwarepath . $fasta_seq_fileB . "-" . $fasta_seq_fileB;

$fasta_seq_fileA = $filePath . $fasta_seq_fileA;                                            
#$fasta_seq_fileB = $filePath . $fasta_seq_fileB;
$fasta_seq_fileB = $softwarepath . $fasta_seq_fileB;
...
print "Read in best hits from blast output file AB done\n";
...
print "Equalize AB scores and BA scores\n";
...
print "Re-sort hits, besthits and bestscore\n";
    for $idA(1..$A){
...
print "Sort orthologs by ID and then by score done\n";
#################################################
...
#		 next unless ($this_family); # There was an alternative BA match but it's best match did not belong here
   ...
   if ($table){
	$filename = $filePath . "table." . $ARGV[0] . "-TARGET";
...
#print "bootstrap pre system\n";
    #system "$blastall -F\"m S\" -i $query_file -z 5000000 -d $db_file -p blastp -M $matrix -m7 | $blastParser 0 -a > $seq_id2.faa";
    my $ttertewtewtewtewt=`$blastall -F\"m S\" -i $query_file -z 5000000 -d $db_file -p blastp -M $matrix -m7 | $blastParser 0 -a > $seq_id2.faa`;

    
    if (-s ("$seq_id2.faa") ){
	my $javaCOmmand="java -jar $seqstat -m $matrix -n 1000 -i $seq_id2.faa > $seq_id2.bs";
	#system("java -jar $seqstat -m $matrix -n 1000 -i $seq_id2.faa > $seq_id2.bs"); # Can handle U, u
	my $javaTempResult=`$javaCOmmand`;	
	if (-s("$seq_id2.bs")){
	...
	print "blast2faa for $query_file / $db_file failed\n"; # if ($debug);                                            
	$significance = 0.0; 
    }
    ...
    unlink "formatdb.log";
    unlink "$seq_id2.fas.psq";
    unlink "$seq_id2.fas.pin";
    unlink "$seq_id2.fas.phr";
    #print "bootstrap after system222222\n";
    return $significance;
}
...
sub do_blast_2pass {
	...
	print "\nStarting first BLAST pass for $qqq - $ttt on ";
	system("date");
	...
	$tmpdir = $filePath;   # May be slightly (5%) faster using the RAM disk "/dev/shm".
	$tmpi = "$tmpdir/tmpi";
	$tmpd = "$tmpdir/tmpd";

	print "\nStarting second BLAST pass for $qqq - $ttt on ";
	system("date");
	unlink "$Fld[4]";
